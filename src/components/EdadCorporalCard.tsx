...
export const EdadCorporalCard: React.FC<EdadCorporalCardProps> = ({
  ...
  // Calcular o tomar edad corporal
  const edadCorp = medicion.edadCorporal && medicion.edadCorporal > 0
  ...
  // Semáforo de edad corporal
  let estadoColor = 'emerald';
  let estadoTitulo = '🟢 Rejuvenecimiento Metabólico';
  let estadoMensaje = `¡Excelente condición! Tu masa muscular y metabolismo hacen que tu cuerpo rinda como el de alguien ${Math.abs(diferencia)} años más joven.`;
  ...
  if (diferencia > 2) {
    estadoColor = 'rose';
    estadoTitulo = '🔴 Alerta de Desgaste Físico';
    ...
    estadoColor = 'amber';
    estadoTitulo = '🟡 Rango Biológico Neutro';
  ...
  return (
    <div className={`rounded-3xl border p-6 sm:p-7 transition-all shadow-xl relative overflow-hidden ${
      isDark 
      ...
      {/* Resplandor de fondo deportivo */}
      <div className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl pointer-events-none ${
        estadoColor === 'emerald' ? 'bg-emerald-500/10' :
        estadoColor === 'rose' ? 'bg-rose-500/10' : 'bg-amber-500/10'
      }`} />
      ...
      {/* Cuerpo Principal: Comparador Dual con Velocímetro / Círculos */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center pt-6">
        
        {/* Lado Izquierdo: Visualización de las 2 Edades (Col 7) */}
        <div className="lg:col-span-7 space-y-5">
          <div className="grid grid-cols-2 gap-4">
            
            {/* Tarjeta Edad Real / Cronológica */}
            ...
            {/* Tarjeta Edad Corporal InBody */}
            <div className={`p-4 sm:p-5 rounded-2xl border text-center relative overflow-hidden transition-all ${
              isDark 
                ? 'bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 border-cyan-500/40 shadow-lg shadow-cyan-500/10' 
                : 'bg-gradient-to-b from-blue-50/50 via-white to-blue-50/30 border-blue-300 shadow-lg shadow-blue-100'
            }`}>
              <div className="absolute top-1.5 right-2">
                <span className="w-2.5 h-2.5 rounded-full bg-cyan-400 animate-ping inline-block" />
              </div>

              <div className="flex items-center justify-center gap-1.5 text-xs text-cyan-500 font-black mb-1">
                <Zap className="w-4 h-4 text-cyan-400" />
                <span>Edad Corporal</span>
              </div>

              <div className="text-4xl sm:text-5xl font-black font-mono tracking-tight text-cyan-400">
                {edadCorp}
                <span className="text-sm font-normal text-cyan-500/80 ml-1">años</span>
              </div>
          ...
          {/* Barra Visual de Rejuvenecimiento / Estado Biológico */}
            ...
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold flex items-center gap-1.5">
                ...
                <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>
                  Equilibrio Biológico InBody
                </span>
              </span>
              <span className={`font-black text-xs ${
                estadoColor === 'emerald' ? 'text-emerald-400' :
                estadoColor === 'rose' ? 'text-rose-400' : 'text-amber-400'
              }`}>
                {esMasJoven ? `⚡ ${Math.abs(diferencia)} años de ventaja biológica` :
                 diferencia > 0 ? `⚠️ ${diferencia} años de desgaste metabólico` :
                 '⚖️ Estado balanceado'}
              </span>
            </div>
            ...
            <div className="w-full h-3.5 bg-slate-800/80 rounded-full overflow-hidden p-0.5 relative">
              <div 
                className={`h-full rounded-full transition-all duration-1000 ${
                  estadoColor === 'emerald' ? 'bg-gradient-to-r from-emerald-500 via-teal-400 to-cyan-400' :
                  estadoColor === 'rose' ? 'bg-gradient-to-r from-rose-600 via-rose-500 to-orange-500' :
                  'bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-400'
                }`}
                style={{ 
                  width: `${Math.min(100, Math.max(15, 100 - (diferencia * 8)))}%` 
                }}
              />
            </div>
        ...
        {/* Lado Derecho: Los 4 Motores que determinan la Edad Corporal (Col 5) */}
        <div className={`lg:col-span-5 p-5 rounded-2xl border space-y-3.5 ${
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {/* Factor 3: Metabolismo Basal TMB */}
            <div className={`p-3 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
              <div className="flex items-center gap-1.5 text-amber-400 text-xs font-bold">
                <Zap className="w-3.5 h-3.5" />
                <span>Gasto Basal TMB</span>
              </div>
              <div className={`text-base font-black font-mono mt-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {medicion.tmb} <span className="text-[11px] font-normal text-slate-400">kcal</span>
              </div>
            ...
            <div className={`p-3 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
              ...
              <span className="text-[10px] font-bold block">
                {medicion.grasaVisceral <= 5 ? '🟢 Óptimo (Protege)' : medicion.grasaVisceral <= 9 ? '🟡 Moderado' : '🔴 Riesgo alto'}
              </span>
            </div>
          </div>
